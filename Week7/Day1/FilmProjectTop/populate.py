...
if __name__ == '__main__':
    ...
    for i in range(99):
        ...
        country=Film(title=movies[i], release_date=radar.random_datetime(),created_in_country=Country.objects.all().get(id=random.randint(0,99)))
        country.save()
        country.available_in_countries.set(available_in_countries)
        country.category.set(categories)
        country.director.set(director)
        country.save()
